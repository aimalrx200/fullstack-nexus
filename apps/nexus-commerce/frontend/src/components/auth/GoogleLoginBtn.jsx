...
export function GoogleLoginBtn({
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`w-full min-h-11 px-4 py-2.5 rounded-xl bg-surface-elevated hover:bg-surface-hover text-text-main border border-border-main text-xs font-semibold flex items-center justify-center gap-2.5 transition-all cursor-pointer disabled:cursor-not-allowed disabled:opacity-60 active:scale-[0.98] disabled:active:scale-100 select-none ${className}`}
    >
      {isLoading ? (
